...
replResultToDoc (LogLevelSet (Just k)) = pure (reflow "Set log level to" <++> pretty0 (show k))
replResultToDoc (ConsoleWidthSet (Just k)) = pure (reflow "Set consolewidth to" <++> pretty0 (show k))
...
displayType defs (n, i, gdef) =
  maybe (do tm <- resugar [] =<< normaliseHoles defs [] (type gdef)
            pure (pretty0 !(aliasName (fullname gdef)) <++> colon <++> prettyBy Syntax tm))
